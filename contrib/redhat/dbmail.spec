...
Source0:        https://hamartun.priv.no/tih/%{name}-%{version}.tar.gz

BuildRoot:      %{buildrootdir}/%{name}-%{version}-%{release}.%{_arch}

BuildRequires:  gmime30-devel > 3.2
...
sed -i -e 's,\(^dburi\W*=\)\(.*$\),\1 sqlite://%{_localstatedir}/lib/dbmail/dbmail.db,' \
       -e 's,\(^logfile\W*=\)\(.*$\),\1 stderr,' \
...
%changelog
* Tue Sep 23 2025 Tom Ivar Helbekkmo <tih@hamartun.priv.no> - 3.5.4-2
- Modified to work well for installation on Redhat RHEL 9
